...
namespace cP
{
    public partial class addPersonnelInfo : Form
    {
        DataGridView dgv;

        public addPersonnelInfo(DataGridView dgv)
        {
            InitializeComponent();
            this.dgv = dgv;
        }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
                   ...
                   == DialogResult.Yes)
            {
                ...
                int result = mainWindow.personnelMap.findInArray(record);
                if (result == -1)
                {
                    string[] infoToGrid = new string[4];
                    infoToGrid[0] = mainWindow.personnelMap.getHash(this.textBox1.Text).ToString();
                    infoToGrid[1] = this.textBox1.Text;
                    infoToGrid[2] = this.textBox2.Text;
                    infoToGrid[3] = this.textBox3.Text;
                    dgv.Rows.Add(infoToGrid);
                    mainWindow.personnelMap.pushBackArray(this.textBox1.Text, this.textBox2.Text, this.textBox3.Text);
                    mainWindow.personnelMap.addHashTable(record);
